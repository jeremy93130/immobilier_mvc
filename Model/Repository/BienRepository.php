...
class BienRepository extends BaseRepository
{
    ...
    public function insertBien(Bien $bien)
    {
        ...
        $request->bindValue(":achatLocation", $bien->getAchatLocation());
    ...
    {
        $sql = "UPDATE bien 
                SET titre = :titre, code_postal = :codePostal, ville = :ville, nb_pieces = :nbPieces,surface= :surface, style = :style, parking = :parking,garage = :garage, prix_vente = :prixVente,loyer_HC = :loyerHC, loyer_CC = :loyer_CC,consommation = :consommation, zone = :zone,ascenseur = :ascenseur, etage = :etage, image = :image, achat_location = :achatLocation
                WHERE id = :id";
        $request = $this->dbConnection->prepare($sql);
